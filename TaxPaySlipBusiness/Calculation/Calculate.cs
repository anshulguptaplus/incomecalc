...
namespace TaxPay.BusinessObjects.Calculation
{
    public static class Calculate
    {
        ...
        {
            if (amount.IsWithin(0, 18200)) return 0.00m;

            if (amount.IsWithin(18201, 37000)) return (amount.CalculateTaxPerDoller(18200, .19m) / 12).RoundOffDoller();

            if (amount.IsWithin(37001, 87000))
                return ((3572 + amount.CalculateTaxPerDoller(37000, .32m)) / 12).RoundOffDoller();

            if (amount.IsWithin(87001, 180000))
                return ((19822 + amount.CalculateTaxPerDoller(87000, .37m)) / 12).RoundOffDoller();

            if (amount >= 180001) return ((54232 + amount.CalculateTaxPerDoller(180000, .45m)) / 12).RoundOffDoller();
